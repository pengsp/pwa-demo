 ...
 */
 ...
 var cacheRequestUrls = [
  ...
  "https://itunes.apple.com/hk/lookup?id="
 ];
 ...
 self.addEventListener('fetch', function (e) {
    ...
    //  console.log('现在正在请求：' + cacheRequestUrls.includes(e.request.url)+ e.request.url);
 
     // 判断当前请求是否需要缓存
    //  var needCache = cacheRequestUrls.some(function (url) {
    //      return e.request.url.indexOf(url) > -1;
    //  });
     let needCache = cacheRequestUrls.includes(e.request.url)
     
     if (needCache) {
      console.log( needCache && '需要缓存')
